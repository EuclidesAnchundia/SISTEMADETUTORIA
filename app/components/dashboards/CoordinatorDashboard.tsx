...
export default function CoordinatorDashboard() {
  ...
  const handleAssignTutor = () => {
    ...
    const existingAssignment = asignaciones.find(
      (a) => a.estudianteEmail === selectedStudent,
    );
  ...
  const handleDeleteAssignment = (
    ...
    if (
      window.confirm(
        `¿Estás seguro de que quieres eliminar la asignación de ${studentName}?`,
      )
    ) {
  ...
  const getAssignedTutor = (studentEmail: string) => {
    ...
    return assignment
      ? tutors.find((t) => t.email === assignment.tutorEmail)
      : null;
  };
  ...
  const getStudentStats = () => {
    ...
    const assignedStudents = students.filter((s) =>
      getAssignedTutor(s.email),
    ).length;
    const studentsWithThemes = students.filter((s) =>
      temas.some((t) => t.estudianteEmail === s.email),
    ).length;
    const activeTutorias = tutorias.filter(
      (t) =>
        students.some((s) => s.email === t.estudianteEmail) &&
        t.estado !== "completada",
    ).length;
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      students.forEach((student, index) => {
        ...
        doc.text(
          `   Tema: ${studentTheme?.titulo || "Sin tema"}`,
          20,
          yPosition + 32,
        );
      ...
      if (tutors.length > 0) {
        ...
        tutors.forEach((tutor, index) => {
          const assignedStudents = students.filter((s) => {
            const assignment = asignaciones.find(
              (a) => a.estudianteEmail === s.email,
            );
            return assignment?.tutorEmail === tutor.email;
          });
          const tutorTutorias = tutorias.filter(
            (t) => t.tutorEmail === tutor.email,
          );
          const completedTutorias = tutorTutorias.filter(
            (t) => t.estado === "completada",
          );

          doc.text(
            `${index + 1}. ${tutor.nombres} ${tutor.apellidos}`,
            20,
            yPosition,
          );
          ...
          doc.text(
            `   Tutorías completadas: ${completedTutorias.length}/${tutorTutorias.length}`,
            20,
            yPosition + 24,
          );
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {students.map((student) => {
                      const assignedTutor = getAssignedTutor(student.email);
                      const assignment = asignaciones.find(
                        (a) => a.estudianteEmail === student.email,
                      );
                      ...
                      return (
                        <tr key={student.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {assignedTutor
                              ? `${assignedTutor.nombres} ${assignedTutor.apellidos}`
                              : "Sin asignar"}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            {studentTheme ? (
                    ...
                    >
                      ...
                      <div className="mt-3 space-y-1">
                        <div className="flex justify-between text-xs">
                          ...
                          <span
                            className={
                              assignedTutor ? "text-green-600" : "text-red-600"
                            }
                          >
                        ...
                        <div className="flex justify-between text-xs">
                          ...
                          <span
                            className={
                              studentTheme ? "text-green-600" : "text-red-600"
                            }
                          >
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-gray-500">
                          {completedTutorias.length}/{tutorTutorias.length}{" "}
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              {temas.filter((t) =>
                ...
                <p className="text-gray-500">
                  No hay temas propuestos en tu facultad.
                </p>
              ) : (
                  ...
                  {temas
                    ...
                    .map((tema) => {
                      const student = students.find(
                        (s) => s.email === tema.estudianteEmail,
                      );
                      const assignedTutor = getAssignedTutor(
                        tema.estudianteEmail,
                      );
                      return (
                        <div
                          key={tema.id}
                        ...
                        >
                          ...
                          {assignedTutor && (
                            <p className="text-sm text-gray-600 mb-2">
                              Tutor asignado: {assignedTutor.nombres}{" "}
                              {assignedTutor.apellidos}
                            </p>
                          )}
                          ...
                          <p className="text-xs text-gray-500">
                            Registrado:{" "}
                            {new Date(tema.fechaRegistro).toLocaleDateString()}
                          </p>
                        </div>
                      );
                    })}
