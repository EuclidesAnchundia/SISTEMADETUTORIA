...
export default function TutorDashboard() {
  ...
  const studentFiles = archivos.filter((a) =>
    assignedStudents.some((student) => student.email === a.estudianteEmail),
  );
  ...
  const updateTutoriaAction = (
    tutoriaId: string,
    updates: Partial<(typeof tutoriaActions)[string]>,
  ) => {
    setTutoriaActions((prev) => ({
      ...prev,
  ...
  const updateThemeAction = (
    themeId: string,
    updates: Partial<(typeof themeActions)[string]>,
  ) => {
    setThemeActions((prev) => ({
      ...prev,
  ...
  const renderStudentDetails = () => {
    ...
    const studentTutorias = tutorias.filter(
      (t) => t.estudianteEmail === selectedStudent.email,
    );
    const studentArchivos = archivos.filter(
      (f) => f.estudianteEmail === selectedStudent.email,
    );

    const completedTutorias = studentTutorias.filter(
      (t) => t.estado === "completada",
    );
    const pendingTutorias = studentTutorias.filter(
            ...
            <div className="space-y-6">
              ...
              {studentTheme ? (
                      ...
                      <div className="space-y-3 pt-4 border-t">
                        {!themeActions[studentTheme.id]?.showRejectForm ? (
                          <div className="flex gap-3">
                            <button
                              onClick={() =>
                                handleThemeAction(studentTheme.id, "aprobar")
                              }
                            ...
                            <button
                              onClick={() =>
                                updateThemeAction(studentTheme.id, {
                                  showRejectForm: true,
                                })
                              }
                          ...
                          <div className="p-4 bg-red-50 rounded-lg">
                            <textarea
                              ...
                              value={
                                themeActions[studentTheme.id]?.observaciones ||
                                ""
                              }
                              onChange={(e) =>
                                updateThemeAction(studentTheme.id, {
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-lg">
                    Este estudiante aún no ha propuesto un tema
                  </p>
                </div>
              )}
            </div>
                ...
                <form onSubmit={handleSendMessage} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Mensaje para {selectedStudent.nombres}{" "}
                      {selectedStudent.apellidos}
                    </label>
                    <textarea
                      value={messageForm.mensaje}
                      onChange={(e) =>
                        setMessageForm({ mensaje: e.target.value })
                      }
                      rows={6}
  ...
  const renderStudentsList = () => {
    const getStudentStats = (studentEmail: string) => {
      const studentTutorias = tutorias.filter(
        (t) => t.estudianteEmail === studentEmail,
      );
      const studentTheme = getTemaByStudent(studentEmail);
      const studentFiles = archivos.filter(
        (a) => a.estudianteEmail === studentEmail,
      );

      return {
        totalTutorias: studentTutorias.length,
        completedTutorias: studentTutorias.filter(
          (t) => t.estado === "completada",
        ).length,
        pendingTutorias: studentTutorias.filter((t) => t.estado === "pendiente")
          .length,
    ...
    const getProgressColor = (stats: any) => {
      if (stats.themeApproved && stats.completedTutorias > 0)
        return "from-green-500 to-green-600";
      if (stats.hasTheme && stats.totalTutorias > 0)
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Gestiona a todos tus estudiantes asignados desde aquí
              </p>
            </div>
          </div>
              ...
              <p className="text-2xl font-bold text-green-600 mt-1">
                {
                  assignedStudents.filter(
                    (s) =>
                      temas.find((t) => t.estudianteEmail === s.email)
                        ?.aprobado,
                  ).length
                }
              </p>
              ...
              <p className="text-2xl font-bold text-yellow-600 mt-1">
                {
                  tutorias.filter(
                    (t) =>
                      t.tutorEmail === user?.email && t.estado === "pendiente",
                  ).length
                }
              </p>
            </div>
            <div className="bg-purple-50 p-4 rounded-lg">
              ...
              <p className="text-2xl font-bold text-purple-600 mt-1">
                {assignedStudents.reduce((total, student) => {
                  return (
                    total +
                    archivos.filter((a) => a.estudianteEmail === student.email)
                      .length
                  );
                }, 0)}
              </p>
          ...
          {assignedStudents.length === 0 ? (
            <div className="col-span-full bg-white rounded-lg shadow-sm p-8 text-center">
              ...
              <p className="text-gray-500">
                Los estudiantes asignados aparecerán aquí cuando el coordinador
                los asigne.
              </p>
            </div>
          ) : (
            assignedStudents.map((student) => {
              ...
              return (
                ...
                >
                  {/* Student Header */}
                  <div
                    className={`bg-gradient-to-r ${getProgressColor(stats)} p-4 rounded-t-lg`}
                  >
                    ...
                    {stats.pendingTutorias > 0 && (
                      <div className="mb-4 p-2 bg-yellow-50 rounded-lg">
                        <p className="text-xs text-yellow-800">
                          ...
                          {stats.pendingTutorias} tutoría
                          {stats.pendingTutorias > 1 ? "s" : ""} pendiente
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-semibold text-gray-900">
                      {
                        tutorTutorias.filter((t) => t.estado === "pendiente")
                          .length
                      }
                    </p>
                  </div>
              ...
              <div className="bg-white p-6 rounded-lg shadow">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-semibold text-gray-900">
                      {
                        tutorTutorias.filter((t) => t.estado === "aceptada")
                          .length
                      }
                    </p>
                  </div>
              ...
              <div className="bg-white p-6 rounded-lg shadow">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-2xl font-semibold text-gray-900">
                      {
                        tutorTutorias.filter((t) => t.estado === "completada")
                          .length
                      }
                    </p>
                  </div>
  ...
  return (
    <div className="border border-gray-200 rounded-lg p-4">
      ...
      <p className="text-sm text-gray-600 mb-2">
        Estudiante: {tutoria.estudianteEmail}
      </p>
      <p className="text-sm text-gray-600 mb-2">
      ...
      {tutoria.descripcion && (
        <p className="text-sm text-gray-700 mb-3">{tutoria.descripcion}</p>
      )}
