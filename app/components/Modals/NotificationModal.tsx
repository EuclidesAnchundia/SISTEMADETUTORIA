...
export default function NotificationModal({ isOpen, onClose }: NotificationModalProps) {
  ...
  useEffect(() => {
    if (isOpen && user) {
      ...
      setNotifications(userNotifications.sort((a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime()))
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div ref={modalRef} className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 max-h-[80vh] flex flex-col">
        {/* Header */}
              ...
              {notifications.map((notification) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className="text-2xl flex-shrink-0">{getNotificationIcon(notification.tipo)}</div>
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm ${!notification.leida ? "font-medium text-gray-900" : "text-gray-700"}`}>
                        {notification.mensaje}
                      </p>
                      <p className="text-xs text-gray-500 mt-1">{formatDate(notification.fecha)}</p>
                    </div>
                    {!notification.leida && (
                      <button
